...
show_next_steps() {
    ...
    echo "   - Root Directory: backend (same as API service)"
    echo "   - Uses backend/railway.json + backend/Dockerfile.prod (shared image)"
    echo "   - Set environment variable: PRDIKTIT_RUN_SCHEDULER=1"
    echo "     (scripts/railway-start.sh runs python -m app.scheduler_minimal when this is set)"
    echo "   - PORT is set by Railway; scheduler health is /health on that port"
