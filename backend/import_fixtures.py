...
os.environ["FOOTBALL_API_KEY"] = "2e229688fb1a199c6243f07a2b6e4b54"
...
# Configuration
UPDATE_EXISTING_FIXTURES = True  # Set to False if you only want new fixtures
            ...
            for league_name, league_config in LEAGUES.items():
                ...
                for fixture_data in fixtures_data:
                    try:
                        ...
                        if existing and UPDATE_EXISTING_FIXTURES:
                            # UPDATE existing fixture
                            existing.home_team = fixture_data['teams']['home']['name']
                            existing.away_team = fixture_data['teams']['away']['name']
                            existing.home_team_logo = fixture_data['teams']['home'].get('logo')
                            existing.away_team_logo = fixture_data['teams']['away'].get('logo')
                            existing.date = fixture_datetime
                            existing.status = status
                            existing.home_score = home_score
                            existing.away_score = away_score
                            existing.round = fixture_data['league'].get('round', 'Round 1')
                            existing.venue = fixture_data['fixture']['venue'].get('name') if fixture_data['fixture'].get('venue') else None
                            existing.venue_city = fixture_data['fixture']['venue'].get('city') if fixture_data['fixture'].get('venue') else None
                            existing.match_timestamp = fixture_datetime
                            existing.last_updated = datetime.now(timezone.utc)
                            
                            logger.debug(f"Updated fixture {fixture_id}: {existing.home_team} vs {existing.away_team}")
                            count += 1
                        ...
                        # If existing and UPDATE_EXISTING_FIXTURES is False, skip (current behavior)
